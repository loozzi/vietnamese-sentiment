...
def train(model, train_loader, val_loader, optimizer, criterion, device, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        #test lấy các chỉ số
        val_loss, correct_preds, total_preds, TP, TN, FP, FN = test_epoch(val_loader, model, optimizer, criterion, device)
        recall_p = TP/(FN + TP)
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}: Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Val Accuracy: {val_accuracy:.2f}%, F1_score: {F1:.2f}")
...
def download_vncorenlp():
    ...
    os.system("wget https://raw.githubusercontent.com/vncorenlp/VnCoreNLP/master/models/wordsegmenter/wordsegmenter.rdr")
...
if __name__ == "__main__":
    ...
    train_ids, train_attention_masks, train_labels = preprocess_data(train_sentences, train_labels, tokenizer, MAX_LEN)
    val_ids, val_attention_masks, val_labels = preprocess_data(val_sentences, val_labels, tokenizer, MAX_LEN)
